...
contract DeployFunWithStorage is Script {
    function run() external returns (funWithStorage) {
        vm.startBroadcast();
        funWithStorage _funWithStorage = new funWithStorage();
        vm.stopBroadcast();
        printStorageData(address(_funWithStorage));
        printFirstArrayElement(address(_funWithStorage));
        return (_funWithStorage);
    }
